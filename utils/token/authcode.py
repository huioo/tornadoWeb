    ...
    def _auth_code(cls, input_string, operation='DECODE', key='', expiry=3600):
        ...
        if rand_key_length:
            if operation == 'DECODE':
                ...
                key_c = cls._md5(str(cls.microtime()))[-rand_key_length:]
        else:
            key_c = ''
        ...
        if operation == 'DECODE':
            print result
            if (int(result[:10]) == 0
                or (int(result[:10]) - int(time.time()) > 0)) \
                    and (result[10:26] == cls._md5(result[26:] + key_b)[:16]):
...
if __name__ == '__main__':
    src = '{"action": "customerByBiYi", "timestamp": "1513732650", "type": "loan", "data": [{"name": "dd"}, {"name": "汤涛"}]}'
    key = 'www.bingru.combiyi2017**'
